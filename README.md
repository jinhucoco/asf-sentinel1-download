...
完整演示「下载 → 配套数据 → AI-InSAR 处理 → 守护查询」全链路对话。

> ⚠️ **示意说明**：对话中的**研究区数据**（77 景/轨道 135/376 对/参数）取自真实 gulang2 实验，但**分组列表、景数、进度数字、AI 措辞**会因你的研究区、搜索时段和实验状态而异——用户可自由换说法，AI 会理解。

```
你: 从 ASF 下载哨兵数据，区域 古浪.shp，时间 20200101 至 20251231，VV
AI: 搜索完成，按(方向,轨道)分组（示意）：
      [1] DESCENDING / 轨道 135: 77 景 ✅ 覆盖研究区
      [2] 其他轨道组: N 景（部分覆盖）
    逐时相覆盖检查后，用轨道 135 吗？(1)
你: 1
...
你: 都用推荐值
AI: 已确认参数 → 执行连接图（376 对）→ 干涉图生成 → 完成后进入反演

你: 实验进展如何
AI: 干涉图生成中（R_6→S_7），磁盘充足，守护监控中，预计按当前速率明早完成
```
...
> 验证：对 AI 说「验证仓库」，AI 跑 34 项验证确保就绪。
