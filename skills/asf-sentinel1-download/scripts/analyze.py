...
from download import load_config, aoi_to_wkt, iso_datetime, parse_polarization
from analysis import (check_per_date_coverage, check_orbit_consistency,
                      check_satellite_consistency, analyze_frame_coverage,
                      sample_by_frequency, export_list, plot_coverage,
                      ask_frequency, ask_rule, FREQUENCY_LABELS)
...
def main():
    ...
    for (d, o), prods in sorted(groups.items(), key=lambda kv: -len(kv[1])):
        ...
        if bad:
            print(f'    ⚠ 混入轨道: {sorted({p.properties.get("pathNumber") for p in bad})}'
                  f'（{len(bad)} 景，应排除）')
        ...
        for fr, info in analyze_frame_coverage(wkt, prods).items():
            print(f'    frame {fr}: {info["n_files"]}景 '
        ...
        # 5. 导出清单
        txt, csv = export_list(prods, os.path.join(args.out, f'list_{d}_{o}.txt'),
                               f'{d} / 轨道 {o}')
        print(f'  清单已导出: {csv}')

        # 6. 覆盖图
        if args.plot:
            img = plot_coverage(wkt, prods, os.path.join(args.out, f'coverage_{d}_{o}.png'),
                                f'{d} / 轨道 {o} 覆盖 vs 研究区', orbit=o)
            print(f'  覆盖图已生成: {img}')

        # 7. 采样（仅对有效轨道组询问，无效组跳过）
        if args.sample and ok_dates:
            ...
            for s in sel[:3]:
                pp = s.properties
                print(f'    {str(pp.get("startTime",""))[:10]} frame{pp.get("frameNumber")} '
                      f'{pp.get("fileName","")[:50]}')
            txt2, csv2 = export_list(sel, os.path.join(args.out, f'sampled_{d}_{o}_{freq}.txt'),
                                     f'{d} / 轨道 {o} {FREQUENCY_LABELS[freq]}采样')
            print(f'    采样清单: {csv2}')
        elif args.sample:
            print(f'  (该轨道组无有效时相，跳过采样)')
