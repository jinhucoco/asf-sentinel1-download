# -*- coding: utf-8 -*-
"""ASF 稳健下载脚本：逐文件下载 + 超时保护 + 自动重试 + 断点续传

增强功能：
  1. 搜索后展示完整数据列表（含帧号、大小），并保存到 inventory.txt
...
RETRY_WAIT = 5                  # 重试间隔（秒）
MAX_FILE_SIZE = 12 * 1024**3    # 单文件大小上限 12GB（SLC 约 4.5GB，留裕量防磁盘耗尽）
ALLOWED_DOWNLOAD_HOSTS = (
    'asf.alaska.edu', 'earthdata.nasa.gov', 'amazonaws.com', 'amazonaws.com.cn',
)
...
def sanitize_filename(fname):
    """文件名消毒：只保留 basename，拒绝路径分隔符/.. /绝对路径。

    防止 ASF 返回恶意文件名时写目录外（路径穿越）。
    """
...
def check_download_url(url):
    """下载 URL 校验：必须 HTTPS 且 host 在白名单内（防 token 泄露/SSRF）。"""
...
def format_inventory_rich(items):
    ...
    for i, it in enumerate(items, 1):
        lines.append(f"{i:<4}{it['date']:<11}{it['orbit']:<8}{it['direction']:<12}"
...
def save_inventory(items, out_dir):
    ...
    with open(path, 'w', encoding='utf-8') as f:
        ...
        f.write('# 轨道组: 方向/轨道 | 文件数: %d | 时相数: %d\n' % (len(items), len(set(it['date'] for it in items))))
        f.write('# 字段: 序号,日期,相对轨道,方向,极化,帧号,文件名\n')
        for i, it in enumerate(items, 1):
            f.write(f"{i},{it['date']},{it['orbit']},{it['direction']},{it['pol']},"
                    f"{it['frame']},{it['file']}\n")
...
def robust_download(product, out_dir, session, progress=None, timeout=DOWNLOAD_TIMEOUT):
    """带超时的单文件下载：分块流式 + 断点续传（Range） + .part 标记 + 完整性校验。

    写入 <name>.part，完成后校验字节数与 md5sum，重命名为正式文件名。
            ...
            with open(part, mode) as f:
                for chunk in resp.iter_content(chunk_size=1024 * 1024):
                    if chunk:
                        ...
                        if progress:
                            progress.update(fname, existing, total_size, 0, 1)
        # 完整性校验：字节数与 asf_search 提供的 md5sum
...
def main():
    ...
    print(f'[OK] 已选择: {ranked[sel_idx-1][0][0]} / 轨道 {ranked[sel_idx-1][0][1]}（{len(results)} 景）')

    # 功能1：展示完整数据列表 + 保存清单
    items = []
    for r in results:
        p = r.properties
        items.append({'date': str(p.get('startTime', ''))[:10].replace('-', ''),
    ...
    # 功能2：启动桌面进度条（GUI 在主线程，下载本就在主线程——改用 queue 方案）
    # 注意：Tk 主循环必须在主线程。这里将 GUI 放入后台线程运行其主循环，
    # 下载线程通过 queue 更新（progress_gui 已做线程安全封装），Windows 下可用；
    # 非 Windows 或失败时自动降级（--no-gui 可强制关闭）。
    progress = None
    if not args.no_gui:
        try:
