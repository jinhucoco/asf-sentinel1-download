# -*- coding: utf-8 -*-
"""Sentinel-1 精密轨道文件（POEORB）下载器 —— 技能正式版。

对应规则：SLC 获取时刻（UTC）必须落在 POEORB 文件的 validity 区间内
...
"""
import csv, os, re, sys, time, zipfile, argparse, urllib.request, glob
...
def main():
    ...
    if args.list:
        ...
        for z in glob.glob(os.path.join(args.data_dir, '*.zip')):
            rows.append({'file': os.path.basename(z), 'satellite': os.path.basename(z).split('_')[0]})
    ...
    for i, ((sat, ymd), hms) in enumerate(timepoints.items(), 1):
        ...
        if not eof:
            log(f'[{i}/{len(timepoints)}] [FAIL] 未找到 {sat} {ymd}T{hms} 的 POEORB（可稍后重跑补漏）', logfile)
            fail += 1
            time.sleep(1)
            continue
        y, m = ymd[:4], ymd[4:6]
