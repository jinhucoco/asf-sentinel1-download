...
for f in required:
    ...
    check('OK' if ok else 'FAIL', 'repo', ok, f'关键文件: {f}',
          '仓库不完整，请检查 clone')
...
check('OK' if not leaked else 'FAIL', 'repo', not leaked,
...
check('OK' if not bad_hard else 'FAIL', 'repo', not bad_hard,
...
check('OK' if not bad_syntax else 'FAIL', 'code', not bad_syntax,
...
# 额外允许的内置/循环变量
builtin_ok = {'ERRORLEVEL', 'PATH', 'TEMP', 'TMP', 'CD', 'DATE', 'TIME', 'RANDOM', 'CMDCMDLINE',
              'PROCESSOR_ARCHITECTURE', 'OS', 'USERNAME', 'USERPROFILE', 'HOMEDRIVE', 'HOMEPATH',
              'PATHEXT', 'COMSPEC', 'PROGRAMFILES', 'PROGRAMDATA', 'WINDIR', 'SYSTEMROOT',
              'NUMBER_OF_PROCESSORS', 'a', 'b', 'x', 'y', 'i'}
bad_var = []
for b in bat_files:
    s = open(b, encoding='utf-8', errors='replace').read()
...
check('OK' if not bad_var else 'FAIL', 'code', not bad_var,
...
if os.path.exists(env_check):
    r = subprocess.run([sys.executable, env_check], capture_output=True, text=True, encoding='utf-8', errors='replace')
    out = r.stdout.strip().split('\n')
    summary = [l for l in out if '结果:' in l or '就绪' in l or '未就绪' in l]
    check('OK' if r.returncode == 0 else 'FAIL', 'env', r.returncode == 0,
...
def run_cli(script, args):
    ...
    r = subprocess.run([sys.executable, p] + args, capture_output=True, text=True, encoding='utf-8', errors='replace')
    return r.returncode == 0

cli_ok = {}
for script, args in [
    ('download.py', ['--help']), ('analyze.py', ['--help']),
    ('robust_download.py', ['--help']), ('multi_download.py', ['--help']),
    ('poeorb_download.py', ['--help']), ('dem_download.py', ['--help']),
    ('gacos_download.py', ['--help']), ('gacos_fetch.py', ['--help']),
]:
...
try:
    ...
    import_ok = False
    import_err = str(e)
check('OK' if import_ok else 'FAIL', 'tool', import_ok,
...
if os.path.exists(cfg_p):
    ...
    # 关键路径存在性
    for key, label, kind in [('WORK_DIR', '工作目录', 'dir'), ('RESULT_ROOT', '输出盘', 'dir'),
                             ('ENVI_IDL', 'ENVI', 'file'), ('SARSCAPE_LIB', 'SARscape', 'dir')]:
        v = cfg.get(key, '')
        if not v:
            continue
        exists = os.path.isdir(v) if kind == 'dir' else os.path.isfile(v)
        check('OK' if exists else 'WARN', 'bat', True if exists else True,
              f'{label} 路径 {"存在" if exists else "不存在"}: {v}')
else:
    check('WARN', 'bat', True, 'config.env 未创建（用户需 copy config.example.env config.env）',
          '全新用户首次运行前必须复制模板')
