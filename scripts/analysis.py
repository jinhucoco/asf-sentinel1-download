...
本次测试沉淀的经验固化为独立分析函数：
1. 逐时相覆盖检查 —— 同一时相（同一天）所有影像并集必须完全覆盖研究区才算有效
...
def check_per_date_coverage(wkt_aoi, products):
    """逐时相覆盖检查：返回 (有效时相, 无效时相)。

    用户核心要求：SBAS 的每个有效时相（同一天），其所有影像（各 frame）
    的 footprint 并集必须【完全覆盖】研究区。只有并集覆盖的时相才可用于
    干涉处理——某时相若只有部分帧（覆盖不全），研究区其他部分在该时相
    缺失，不能使用。
...
def check_orbit_consistency(products):
    """检查组内所有影像的轨道号（pathNumber）是否完全一致。
...
def check_satellite_consistency(products):
    """检查组内所有影像的卫星（platform）是否一致。

    返回 (一致?, 卫星集合)。背景：Sentinel-1 任务从 S1A/S1B 过渡到 S1C，
    不同卫星的传感器特性有差异，直接混入同一 SBAS 序列需谨慎。
    """
...
def analyze_frame_coverage(wkt_aoi, products):
    ...
    返回 {frame: {n_files, first_date, last_date, cover_ratio, fully_covers}}
    cover_ratio: 该 frame 影像覆盖研究区的面积比例（0~1）
    fully_covers: 是否单帧 100% 覆盖研究区
    """
    from shapely.geometry import shape
    from shapely.wkt import loads
...
def monthly_sample(products, rule='first'):
    ...
    rule: 'first' 取每月最早时相；'middle' 取月中时相；'last' 取月末时相。
    每月选中的时相包含该时相的所有帧（跨帧时相上下景都保留）。
    """
...
def sample_by_frequency(products, frequency='monthly', rule='first'):
    ...
    frequency:
        ...
        'semiyearly' — 每半年一个时相
        'yearly'     — 每年一个时相
    rule: 'first' 取最早时相；'middle' 取中间时相；'last' 取最晚时相。
    每个采样区间选中的时相包含该时相的所有帧（跨帧时相上下景都保留）。
    """
    if frequency == 'all':
        return list(products)

    # 按区间分组：monthly→YYYY-MM, quarterly→YYYY-Qn, semiyearly→YYYY-Hn, yearly→YYYY
    def bucket(ymd):
...
FREQUENCY_LABELS = {
    'all': '全部（不采样）', 'monthly': '每月', 'quarterly': '每季度',
    'semiyearly': '每半年', 'yearly': '每年',
}
...
def ask_rule():
    """交互式询问时相选择规则。返回 'first'/'middle'/'last'。"""
    print('\n每个区间取哪个时相？')
    print('  [1] 最早时相')
    print('  [2] 中间时相')
    print('  [3] 最晚时相')
...
def export_list(products, out_path, title='ASF 下载清单'):
    ...
    with open(out_path, 'w', encoding='utf-8') as f:
        ...
        for r in rows:
            f.write(f"{r['date']},{r['frame']},{r['orbit']},{r['satellite']},{r['file']}\n")
    # CSV
...
def plot_coverage(wkt_aoi, products, out_path, title='影像覆盖 vs 研究区', orbit=None):
    """生成研究区 vs 各 frame 影像覆盖范围的可视化图。

    关键修复：按 (轨道号, frame) 组合分组绘制——同一 frame 编号可能被
    不同轨道复用（如 frame 468 在轨道 135 与 62 下 footprint 完全不同），
    必须按轨道严格区分，避免画错/漏画。
    out_path: .png 路径
    orbit: 指定只画某个轨道的影像（推荐，避免多轨道混杂）
    """
    import matplotlib
    matplotlib.use('Agg')
    ...
    for i, ((path, fr), prods) in enumerate(combo_list):
        ...
        ax.text(cx, cy, f'{fr}', fontsize=12, ha='center', va='center',
