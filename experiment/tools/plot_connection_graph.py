...
def parse_cg(cg_dir):
    ...
    for line in content.split('\n'):
        if 'Number of images' in line:
            ...
            continue
        if in_images:
            m = re.search(r'sentinel1_135_(\d{8})_\S+.*?ID : \[(\d+)\].*?Valid : \[(\w+)\].*?Position : \[([-\d.]+)\]', line)
            if m:
                acq.append({'date': m.group(1), 'id': int(m.group(2)),
                            'valid': m.group(3) == 'YES', 'pos': float(m.group(4))})
    ...
    for line in content.split('\n'):
        ...
        if rm:
            ref_date = rm.group(1)
            continue
        sm = re.search(r'SECONDARY : \d+ .*sentinel1_135_(\d{8})_.*?NormalBaseline = \[([-\d.]+)\].*?TemporalBaseline = \[(-?\d+)\]', line)
        if sm and ref_date:
            pairs.append({'ref': ref_date, 'sec': sm.group(1),
                          'nb': float(sm.group(2)), 'tb': int(sm.group(3))})
    return acq, pairs, sr
...
def plot_time_baseline(acq, pairs, sr, out_png):
    ...
    for p in pairs:
        ...
        ax.plot([d1, d2], [p['nb'], p['nb']], '-', color='#4472C4',
    ...
    for a in acq:
        ...
        if a['date'] == sr['date']:
            ...
            zorder = 5
        ax.plot([d], [0], marker='D', markersize=size, color=color,
                markeredgecolor=ec, markeredgewidth=0.9, zorder=zorder)
        ax.annotate(str(a['id']), (d, 0), textcoords='offset points',
                    xytext=(0, -19), ha='center', fontsize=7.5,
                    color='#555555', zorder=7)

    ax.axhline(0, color='#999999', linewidth=1.0)
    # 基线范围: 数据集中在±60m, 显示±150m
    ax.set_ylim(-150, 150)
    style_ax(ax, 'Acquisition Date', 'Normal Baseline [m]',
             f'Time-Baseline Plot  (Super Reference: {sr["date"]})')

    # 图例 (右上角避免遮挡)
    legend_elems = [
        Line2D([0], [0], marker='D', color='w', markerfacecolor='#FFC000',
               markeredgecolor='black', markersize=11, label=f'Super Reference ({sr["date"]})'),
        Line2D([0], [0], marker='D', color='w', markerfacecolor='#00B050',
               markeredgecolor='black', markersize=11, label='Valid Acquisition'),
        Line2D([0], [0], marker='D', color='w', markerfacecolor='#FF0000',
               markeredgecolor='black', markersize=11, label='Discarded Acquisition'),
        Line2D([0], [0], color='#4472C4', linewidth=1.5, label=f'Interferometric Pair ({len(pairs)})'),
    ]
    ax.legend(handles=legend_elems, loc='upper center', fontsize=10, framealpha=0.92,
              edgecolor='#cccccc', ncol=4, bbox_to_anchor=(0.5, 1.10))
    fig.autofmt_xdate()
    fig.tight_layout()
...
def plot_time_position(acq, pairs, sr, out_png):
    ...
    for a in acq:
        ...
        if a['date'] == sr['date']:
            ...
            zorder = 5
        ax.plot([d], [a['pos']], marker='D', markersize=size, color=color,
                markeredgecolor=ec, markeredgewidth=0.9, zorder=zorder)
        ax.annotate(str(a['id']), (d, a['pos']), textcoords='offset points',
    ...
    style_ax(ax, 'Acquisition Date', 'Relative Position [m]',
