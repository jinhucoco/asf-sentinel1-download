...
cd /d %WORK_DIR%
"%ENVI_IDL%" -quiet -e "!PATH=!PATH+';'+'%SARSCAPE_LIB%\envi_extensions\idl\lib'+';'+'%SARSCAPE_LIB%\envi_extensions\idl\lib\hook'+';'+'%SARSCAPE_LIB%\envi_extensions\envi\sarscape_local_sav' & resolve_routine,'sarscape_batch_init',/COMPILE_FULL_FILE & SARscape_Batch_Init,Temp_Directory='%WORK_DIR%\sar_tmp' & oSB=obj_new('SARscapeBatch') & openw,u,'%SAR_MODULES%',/get_lun & printf,u,'PRE-SEARCH' & r=oSB.SearchModule('SENTINEL') & printf,u,'SENTINEL:',strjoin(r,'|') & r2=oSB.SearchModule('SBAS') & printf,u,'SBAS:',strjoin(r2,'|') & free_lun,u & exit" > sarbatch_err11.txt 2>&1
echo EXIT=%ERRORLEVEL% >> sarbatch_err11.txt
