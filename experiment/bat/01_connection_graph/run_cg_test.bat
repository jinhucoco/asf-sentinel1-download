...
cd /d %WORK_DIR%
"%ENVI_IDL%" -quiet -e "!PATH=!PATH+';'+'%SARSCAPE_LIB%\envi_extensions\idl\lib'+';'+'%SARSCAPE_LIB%\envi_extensions\idl\lib\hook'+';'+'%SARSCAPE_LIB%\envi_extensions\envi\sarscape_local_sav' & resolve_routine,'sarscape_batch_init',/COMPILE_FULL_FILE & SARscape_Batch_Init,Temp_Directory='%WORK_DIR%\sar\tmp' & openw,u,'%SAR_MODULES%',/get_lun & o=obj_new('SARscapeBatch',Module='InSARStackSBASGenerateConnectionGraph') & printf,u,'OBJ:',byte(OBJ_VALID(o)) & a=o.SetParam('input_file_list','%WORK_DIR%\sar\slc_list_arr.txt') & printf,u,'SETIN_FILE:',byte(a) & free_lun,u & exit" > sarbatch_cg_test.txt 2>&1
echo EXIT=%ERRORLEVEL% >> sarbatch_cg_test.txt
