...
class Guardian:
    ...
    def restart(self):
        ...
        try:
            popen_hidden(['cmd', '/c', self.bat_file],
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                if not alive:
                    if os.path.exists(os.path.join(WORKDIR, 'pause.flag')) or os.path.exists(os.path.join(WORKDIR, 'stop.flag')):
                        ...
                        if self._restart_count >= 3:
                            ...
                            wake_ai(f'反复崩溃（10 分钟内 {self._restart_count} 次）已停止自动重启。'
                            ...
                            notify_wechat(f'干涉进程消失，自动重启(第{self._restart_count}次)',
                                          f'异常信息: {err or "无错误标记"}\n已拉起新进程。')
                            wake_ai(f'实验进程消失，守护自动重启第 {self._restart_count} 次。'
                    ...
                    # ===== CPU 卡死检测（进程在但没在算）=====
                    cpu = cpu_seconds()
                    if cpu >= 0 and self._last_cpu >= 0 and (now - self._last_cpu_time) >= CPU_STALL_CHECK_MIN * 60:
                    ...
                    cpu_now = cpu_seconds()
                    cpu_growth = (cpu_now - self._last_cpu) if (cpu_now >= 0 and self._last_cpu >= 0) else -1
                    ...
                    if t and (now - t) > STALL_MIN * 60 and cpu_growth < 5:
                        ...
                        if not self._reported_r38_done and prog and 'R_38' not in prog and 'R_38' in self._last_progress:
                    ...
                    if now - self.last_health >= HEALTH_CHECK_MIN * 60:
                        ...
                        log(f'[体检] {done_cnt}/{total} 对 ({pct:.1f}%), {output_size_gb()}, 进度: {prog_now}')
                ...
                free = disk_free_gb()
                if free < 20:
                    notify_wechat('磁盘空间不足！', f'结果盘仅剩 {free:.0f}GB，SBAS 处理可能失败，请尽快处理。')
                ...
                if mail_report_enabled() and now - self.last_report >= 7200 and in_report_hours():
                    ...
                    send_mail(f'[SBAS] 定时进度汇报 {time.strftime("%H:%M")}', full_report())
                      ...
                      and time.localtime().tm_min == 0 and now - self.last_report >= 1800):
                    self.last_report = now
