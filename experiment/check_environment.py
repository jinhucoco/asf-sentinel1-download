...
if not config_loader.CONFIG_FILE or not os.path.exists(config_loader.CONFIG_FILE):
    check('FAIL', 'config', False, 'config.env 不存在',
...
for dep in DEPS:
    ...
    check('OK' if ok else 'FAIL', 'pip', ok, f'{dep}（pyshp 的模块名是 shapefile）' if dep == 'shapefile' else dep, 'pip install ' + dep)
# 可选依赖
...
def check_path(cfg_key, label, kind='dir', optional=False):
    v = CFG.get(cfg_key, '')
    if is_template(v):
        check('FAIL' if not optional else 'WARN', 'path', False,
    ...
    if exists:
        ...
        check('FAIL', 'path', False, f'{label}: 不存在: {v}',
              f'确认 {cfg_key} 指向真实路径')


check_path('WORK_DIR', '工作目录')
check_path('SLC_DATA', 'SLC 数据目录', optional=True,
           )  # 未下载数据前可空，下载工具会创建
...
def check_soft(cfg_key, label, kind='file'):
    v = CFG.get(cfg_key, '')
    if is_template(v):
        check('FAIL', 'soft', False, f'{label}: 未配置 ({cfg_key})',
    ...
    if exists:
        ...
        check('FAIL', 'soft', False, f'{label}: 找不到: {v}',
...
if sc and os.path.isdir(sc):
    ...
    if missing:
        check('FAIL', 'soft', False, f'SARscape 扩展子目录缺失: {missing}',
...
def check_import(mod_name, path, label):
    try:
        ...
        check('FAIL', 'script', False, f'{label} 导入失败: {e}',
              '确认 pip 依赖已装（见上）')
...
try:
    ...
    if free > 20:
        ...
        check('FAIL', 'disk', False, f'结果盘空间不足 {free:.0f}GB: {disk_root}',
