...
echo "  🔑 安装后请配置好以下账户（按需）:"
echo "     [必需] NASA Earthdata 账号 → 编辑 config.json（SLC 下载 + DEM 下载都需要）"
echo "           免费注册: https://urs.earthdata.nasa.gov/ ，对话中说「配置 ASF 账号密码」可交互式配置"
echo "     [可选] 邮箱（GACOS 大气延迟）→ 一个能收邮件的邮箱（163/QQ），结果通过邮件发送；"
echo "           自动收件需在邮箱开启 IMAP 并获取授权码"
echo "     [免账号] POEORB 精密轨道 → ESA 公开服务器，无需任何账号"
echo "  💡 未配置 Earthdata 账号时，SLC/DEM 下载会认证失败"
echo ""
echo "====================================================="
