# -*- coding: utf-8 -*-
"""GACOS 大气延迟数据提交下载器 —— 技能正式版（Python + Playwright）。

GACOS 无 API，只能网页表单提交（限制：每次 ≤20 日期）。本脚本自动填表提交，
结果通过邮件发送，用 gacos_fetch.py 收取。
...
def main():
    ...
    ap.add_argument('--bbox', required=True, help='"N W E S"（最大纬度 最小经度 最大经度 最小纬度）')
    ...
    with sync_playwright() as pw:
        ...
        for i, batch in enumerate(split_batches(dates), 1):
            ...
            page.goto('http://www.gacos.net/index.html', wait_until='domcontentloaded', timeout=120000)
